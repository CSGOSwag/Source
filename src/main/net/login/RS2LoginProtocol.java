...
public class RS2LoginProtocol extends FrameDecoder {
	...
	protected Object decode(ChannelHandlerContext ctx, Channel channel,
		...
		case LOGGING_IN:
			...
			int clientVersion = buffer.readShort(); //TA INTE BORT (tror det var det som fuckade upp RSA) 
			...
			final String mac = Misc.getRS2String(buffer);
			channel.getPipeline().replace("decoder", "decoder",
					new RS2Decoder(inCipher));
			return login(channel, inCipher, outCipher, version, name, pass, mac);
		}
		return null;
	}
	...
	private static Client login(Channel channel, ISAACCipher inCipher,
			ISAACCipher outCipher, int version, String name, String pass, String mac) {
		int returnCode = 2;
		//System.out.println(mac);
		String IP = ((InetSocketAddress) channel
				.getRemoteAddress()).getAddress().getHostAddress().toString();
		if (Connection.isIpBanned(IP)) {
			returnCode = 4;
		}
