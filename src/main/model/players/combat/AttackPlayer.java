...
public class AttackPlayer {
	...
	public static void attackPlayer(Client c, int i) {
		if (PlayerHandler.players[i] != null) {
			...
			if (c.inWild() && c.playerEquipment[c.playerWeapon] == 11907 || c.playerEquipment[c.playerWeapon] == 12899) {
				c.sendMessage("You can't use the trident against another player in the wild.");
				return;
			}
			if (c.inMithWild() && c.playerEquipment[c.playerWeapon] == 11907 || c.playerEquipment[c.playerWeapon] == 12899) {
				c.sendMessage("You can't use the trident against another player in the wild.");
				return;
			}
			...
			if (c.attackTimer <= 0) {
				...
				if ((!c.goodDistance(c.getX(), c.getY(),
						...
						|| (!c.goodDistance(c.getX(), c.getY(),
								PlayerHandler.players[i].getX(),
								PlayerHandler.players[i].getY(), 10) && (c.usingBow || c.usingMagic || c.usingToxicBlowpipe))) {
					c.attackTimer = 1;
					if (!c.usingBow && !c.usingMagic
							&& !c.usingOtherRangeWeapons && !c.usingToxicBlowpipe && c.freezeTimer > 0)
						c.getCombat().resetPlayerAttack();
					return;
				}
				...
				if (c.playerEquipment[c.playerWeapon] == 11785 && !c.getCombat().properBolts()) {
					c.sendMessage("You must use bolts with a crossbow.");
					c.stopMovement();
					c.getCombat().resetPlayerAttack();
					return;
				...
				if (c.usingBow || c.usingMagic || c.usingOtherRangeWeapons || c.usingToxicBlowpipe
				...
				c.lastArrowUsed = 0;
				c.rangeItemUsed = 0;
				if (!c.usingBow && !c.usingMagic && !c.usingOtherRangeWeapons && !c.usingToxicBlowpipe) { // melee
																					// hit
																					// delay
				...
				if (c.usingBow && !c.usingOtherRangeWeapons && !c.usingMagic && !c.usingToxicBlowpipe
