...
public class ClanManager {
	...
	public void create(Client paramClient) {
		if (Player.clan != null) {
			paramClient.sendMessage("You must first leave the clan you are in.");
			return;
		}
		...
		paramClient
				.sendMessage("You may change your clan settings by clicking the 'Clan Setup' button.");
	}
	...
	public void delete(Clan paramClan) {
		...
		File localFile = new File("data/clan/" + paramClan.getFounder()
				+ ".cla");
		if (localFile.delete()) {
			Client localClient = (Client)PlayerHandler.getPlayer(paramClan.getFounder());
			if (localClient != null) {
