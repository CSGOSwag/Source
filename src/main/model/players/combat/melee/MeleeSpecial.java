package main.model.players.combat.melee;
	...
	public static void activateSpecial(Client c, int weapon, int i){
		...
		switch(weapon) {
	        ...
	        if (c.playerIndex > 0) {
	        	...
	        	if (Misc.random(c.getCombat().calculateMeleeAttack()) > Misc.random(o.getCombat().calculateMeleeDefence())) {
	        		c.clawDamage = Misc.random(c.getCombat().calculateMeleeMaxHit() + Misc.random(3));//4
	        	}
	        	c.clawIndex = c.playerIndex;
	        	c.clawType = 1;
	        } else if (c.npcIndex > 0) {
	        	NPC n = NPCHandler.npcs[c.npcIndex];
	        	if (Misc.random(c.getCombat().calculateMeleeAttack()) > Misc.random(n.defence)) {
	        		c.clawDamage = Misc.random(c.getCombat().calculateMeleeMaxHit() + Misc.random(1));//2
	        	}
