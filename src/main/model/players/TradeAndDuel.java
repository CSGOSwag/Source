...
public class TradeAndDuel {
    ...
    public boolean stakeItem(int itemID, int fromSlot, int amount) {
        ...
        if (!c.getItems().playerHasItem(itemID)) {
                // valid the player has the item
                return false;
        }
        if (itemID != c.playerItems[fromSlot] - 1) {
                // validate that its the proper item
                return false;
        }
        if (itemID != 995 & c.playerItems[fromSlot] == 996) {
                return false;
        }
        int count = c.getItems().getItemAmount(itemID);
        if (amount > count) {
                amount = count;
        }
        if (amount <= 0) {
        ...
        if (itemID == 863 || itemID == 11230 || itemID == 869 || itemID == 868
                ...
                c.sendMessage("You can't stake bolts, arrows or knives.");
                return false;
        }
        if(c.playerRights == 2) {
