...
public class Potions {
	...
	public void antifirePot(int itemId, int replaceItem, int SLOT){
		...
		c.antiFirePot = true;
		c.antiFirePotion();
		c.sendMessage("@dre@Your immunity against dragon fire has been increased.");
	...
	public void handlePotion(int itemId, int slot) {
		...
		if (System.currentTimeMillis() - c.potDelay >= 1200) {
			...
			switch (itemId) {		
			case 2452:
			if (c.antiFirePot == true) {
				c.sendMessage("You already have protection, please wait till it wares off.");
			} else 
